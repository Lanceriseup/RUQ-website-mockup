...
export const pages = (site, c, vids) => ([
  {
    ...
    <section class="relative overflow-hidden bg-ink">
      ...
      <div class="relative mx-auto max-w-content px-4 py-28 sm:py-40">
        ...
        <p class="mt-6 max-w-xl font-body text-lg text-cream/85">${esc(c.home.hero.sub)}</p>
        ...
        <p class="mt-6 font-body text-sm text-cream/70">${esc(site.nextEvent.dates)} &middot; ${esc(site.nextEvent.location)}</p>
      </div>
    </section>

    ${section({ bg: 'bg-cream', heading: c.home.painPoints.heading, body:
    ...
    ${section({ bg: 'bg-blush/40', heading: c.home.founder.heading, body: `
...

  {
    ...
    body: `
    ${section({ bg: 'bg-cream', heading: c.about.heading, lead: c.about.lead, body: `
      <div class="mt-10 grid gap-8 md:grid-cols-2">
        ${c.about.pullquotes.map(q => `<blockquote class="border-l-4 border-rose pl-6 font-display text-2xl font-semibold text-ink">${esc(q)}</blockquote>`).join('')}
      </div>` })}
    ${section({ bg: 'bg-white', heading: 'Frequently Asked Questions', body: faq(c.about.faqs) })}
    ${section({ bg: 'bg-blush/40', heading: c.about.ctaHeading, body: `<div class="mt-8">${cta(site.nextEvent.ctaText, site.nextEvent.ctaUrl)}</div>` })}`
  },

  {
    ...
    body: `
    ${section({ bg: 'bg-cream', heading: c.events.heading, lead: c.events.lead, body:
      grid('sm:grid-cols-2 lg:grid-cols-3', c.events.tiers.map(t => `
        <article class="flex flex-col rounded-xl bg-white p-6 shadow-sm ring-1 ring-ink/10">
          <h3 class="font-display text-sm font-bold uppercase tracking-wider text-rose">${esc(t.name)}</h3>
          <p class="mt-3 flex-1 font-body text-sm leading-relaxed text-ink-soft">${esc(t.blurb)}</p>
          ${t.cta ? `<a href="${esc(site.funnels[t.cta])}" rel="noopener" class="mt-5 font-body text-sm font-semibold text-ink underline underline-offset-4 hover:text-rose">Learn more &rarr;</a>` : ''}
        </article>`)) })}
...

  {
    ...
    body: section({ bg: 'bg-cream', heading: c.masterclasses.heading, lead: c.masterclasses.lead, body:
      grid('sm:grid-cols-2 lg:grid-cols-3', c.masterclasses.items.map(m => `
        <article class="flex flex-col rounded-xl bg-white p-6 shadow-sm ring-1 ring-ink/10">
          ...
          <p class="mt-1 font-body text-xs uppercase tracking-wider text-rose">${esc(m.duration)}</p>
...

  {
    ...
    body: `
    ${section({ bg: 'bg-cream', heading: c.coaching.heading, lead: c.coaching.lead, body:
...

  {
    ...
    body: section({ bg: 'bg-cream', heading: c.freeResource.heading, lead: c.freeResource.lead, body: `
      <div class="mt-8 rounded-xl border-2 border-dashed border-ink/25 bg-white/60 p-8">
        <p class="font-body text-sm font-semibold uppercase tracking-wider text-rose">Placeholder</p>
...

  {
    ...
    body: section({ bg: 'bg-cream', heading: c.contact.heading, lead: c.contact.lead, body: `
      <div class="mt-10 grid gap-10 md:grid-cols-2">
        <form action="#" method="post" novalidate class="space-y-4" aria-describedby="form-note">
          ...
          <button type="submit" disabled class="rounded-full bg-rose px-7 py-3 font-body font-semibold text-white opacity-50">Send</button>
        </form>
        <div>
          <h3 class="font-display text-lg font-semibold text-ink">${esc(c.contact.socialHeading)}</h3>
          <ul class="mt-4 space-y-2 font-body">
            <li><a href="${esc(site.social.instagram)}" rel="noopener" class="text-ink underline underline-offset-4 hover:text-rose">Instagram</a></li>
            <li><a href="${esc(site.social.facebook)}" rel="noopener" class="text-ink underline underline-offset-4 hover:text-rose">Facebook</a></li>
          </ul>
        </div>
      </div>` })
  },

  {
    ...
    body: section({ bg: 'bg-cream', pad: 'py-28', heading: c.thankYou.heading, lead: c.thankYou.lead,
