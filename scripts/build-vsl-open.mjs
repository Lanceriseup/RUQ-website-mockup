...
import { VSL_OPEN } from '../src/partials/vsl-open.mjs';
...
<div class="mx-auto max-w-content px-4 py-10">
  ...
  <div class="mt-4 rounded-xl bg-magenta-tint p-5 text-sm">
    ...
    <br><br>
    <strong>There is something real to open into.</strong> The frame sits at 2.39:1 over a 16:9 source, so 25.6% of the
    picture is hidden behind two letterbox bars — 12.81% at each end. That is what
    <span class="font-semibold">unfold</span> gives back, and it is the only option whose end state is a different shape
    from its start.
  </div>

  <div class="mt-3 rounded-xl border border-amber-300 bg-amber-50 p-5 text-sm">
    <strong>The bars scale, they do not resize.</strong> Height is a layout property; animating it would reflow the hero on
    every frame of the transition. They use transform: scaleY from their outer edge instead, which stays on the
    compositor.
    <br><br>
    <strong>All six are off under prefers-reduced-motion</strong> — the stage jumps straight to its playing state. The
...
console.log(`built vsl-open.html + ${Object.keys(VSL_OPEN).length} frames`);
